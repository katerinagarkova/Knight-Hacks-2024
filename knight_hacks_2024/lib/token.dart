...
Future<void> markDone(String taskId) async {
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      print('Error marking event as done: ${response.statusCode}');
    }
  } catch (error) {
    print('$error');
  }
...
Future<String> addEvent(
    String event, DateTime date, int userID) async {
  const String apiUrl =
      'https://locked-in-561ee2a901c9.herokuapp.com/api/calendar/create';

  final Map<String, dynamic> requestBody = {
    'Event': event,
    'StartTime': "12", // Ensure this object is structured correctly
  ...
  try {
    ...
    if (response.statusCode == 200) {
      final jsonResponse = json.decode(response.body);
      final eventId = jsonResponse['EventID'] as String;
      return eventId;
      throw 'Event added successfully';
    } else {
      throw 'Failed to add event. Status Code: ${response.statusCode}';
    }
  } catch (error) {
    throw '$error';
  }
...
Future<void> deleteEvent(String eventID) async {
  const baseUrl =
      'https://locked-in-561ee2a901c9.herokuapp.com'; // Replace with your API base URL
  final endpoint = '/api/event/delete/$eventID';
  ...
  try {
    ...
    print('An error occurred while deleting the event: $error');
  }
}

Future<void> updateEvent(String eventId, String newText) async {
  print(newText);
  final baseUrl =
      'https://locked-in-561ee2a901c9.herokuapp.com'; // Replace with your API base URL
  final endpoint = '/api/event/update/$eventId';

  final updateFields = {'Event': newText}; // Construct update fields
  ...
  try {
    ...
    if (response.statusCode == 200) {
      throw ('Event updated successfully: ${response.statusCode}');
    } else if (response.statusCode == 404) {
      throw ('Event not found');
    } else if (response.statusCode == 304) {
      throw ('Event not modified');
    } else {
